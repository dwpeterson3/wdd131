...
function validateForm(event) {
    ...
    if (theForm.cardnumber.value !== '1234123412341234'){
        isValid = false;
        errors.push('Your number is not 1234123412341234');
    }
...
function togglePaymentDetails(e) {
    ...
    theForm.cardnumber.removeAttribute('required');
    ...
    if (e.target.value === 'credit') {
        ...
        theForm.cardnumber.setAttribute('required', cardnumber);
    }
    else if (e.target.value === 'paypal') {
        paypalContainer.classList.remove('hide');
        theForm.paypal.setAttribute('required', paypal);
    }
}
